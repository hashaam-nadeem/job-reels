...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  @override
  void initState() {
    if (Get.find<AuthController>().getLoginUserData()!.type.toString() ==
            "admin" &&
        widget.isMyProfile == true) {
      getAdminUserProfileData();
    } else {
      getUserProfileData();
    }
    super.initState();
  ...
  Future getAdminUserProfileData() async {
      ...
      setState(() {
        myPostedVideosList =
            List<Post>.from(result['posts'].map((e) => Post.fromJson(e)))
                .toList();
        print("my post length: ${myPostedVideosList.length}");
      });
      // }
      // if (widget.isMyProfile) {
  ...
  Widget build(BuildContext context) {
    ...
    // print ("my videos: ${myPostedVideosList.length}");
    return Scaffold(
                    ...
                    ? Column(
                        children: [
                          ...
                          InfoWidget(
                              icon: Entypo.info,
                              title: AppString.bio,
                              description: "${AppString.adminAccountBio}"),
                          const SizedBox(
                            height: Dimensions.PADDING_SIZE_DEFAULT,
                          ),
                          const InfoWidget(
                                                      ...
                                                      onPressed: () {
                                                        if (Get.find<
                                                          ...
                                                          showLoginMessagePopup();
                                                                    ...
                                                                    AuthController>()
                                                                .isLoginUserSubscribed ||
                                                            isAdminProfile) {
                                                          Get.find<
                                                                  ChatNotificationController>()
                                                              .fetchThreadAndGoToChatScreen(
                                                                  user!.id!);
                                                        } else {
                                                          if (Get.find<
                                                                      AuthController>()
                                              ...
                                              : myPostedVideosList.isNotEmpty
                                                  ...
                                                  : otherUsersSavedPosts
                                                      ...
                                                      : SizedBox(
                                                          ...
                                                          child: Center(
                                                            child: Text(
                                                              '${widget.isMyProfile ? "You have" : "This user has"} not ${(user?.isFreelancer ?? false) ? "posted" : "saved"} any video!',
                                                              textAlign:
                                                                  TextAlign
                                                                      .center,
                                                              style:
  ...
  Widget adminProfileView() {
    // print("my user phone no: ${myUser!.phone.toString()}");
    return Column(
      ...
      children: [
        ...
        InfoWidget(
            icon: Entypo.info,
            title: AppString.bio,
            description: isAdminProfile
                ? AppString.adminAccountBio
  ...
  Widget freeLancerView() {
    // print("free lancer view: ${myUser!.type}");
    return Column(
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        Row(
          ...
          children: [
            SizedBox(
              ...
              child: Wrap(
                ...
                children: user!.freelancer!.skillsAssessment
                    .map((skill) => Container(
        ...
        user!.freelancer!.description.toString().isEmpty ||
                user!.freelancer!.description.toString() == "null"
            ? Container(
                width: 0,
                height: 0,
              )
        ...
        InfoWidget(
            ...
            title: AppString.expectedSalaryMonthly,
            description:
                "${user!.freelancer!.salaryRequirements ?? ""}(\$/month)"),
        const SizedBox(
          height: Dimensions.PADDING_SIZE_SMALL,
        ),

        InfoWidget(
            icon: AntDesign.clockcircle,
            title: AppString.availability,
            description: user!.freelancer!.fullTime),
        const SizedBox(
          height: Dimensions.PADDING_SIZE_SMALL,
        ),
        InfoWidget(
            icon: MaterialIcons.work,
            title: AppString.yearsOfExperience,
            description: user!.freelancer!.yearsExperience),
        ...
        GestureDetector(
          onTap: () {
            print("user type is : ${authController.isLoginUserSubscribed}");
            if (Get.find<AuthController>().getLoginUserData()?.isFreelancer ??
                true) {
            } else if (Get.find<AuthController>().authRepo.isLoggedOut()) {
            } else {
              if (authController.isLoginUserSubscribed) {
              } else {
                if (Get.find<AuthController>()
                        .getLoginUserData()!
                        .type
                      ...
                      Get.find<AuthController>().authRepo.isLoggedOut()
                          ...
                          : widget.isMyProfile
                              ...
                              : Get.find<AuthController>()
                                          ...
                                          ?.isFreelancer ??
                                      true
                                  ? "you cannot view contact information"
                                  : !authController.isLoginUserSubscribed
                                      ? "Please subcribe to see contact information (free for now)"
                                      : "Contact Information",
                      // ? "you cannot view contact information"
                      // : !authController.isLoginUserSubscribed
                      //     ?
        ...
        Get.find<AuthController>().authRepo.isLoggedOut()
            ...
            : widget.isMyProfile
                ? socialMediaLinks()
                : Get.find<AuthController>().getLoginUserData()?.isFreelancer ??
                        true
                    ? Container(
                        height: 0,
                        width: 0,
  ...
  Widget socialMediaLinks() {
    ...
    return Column(
      children: [
        if (user!.phone != null && !user!.isPhoneNumberHide)
  ...
  bool get isAdminProfile => user!.id == 165;
  bool get isNotAdminProfile => user!.id != 165;

  Widget hirerView() {
    print("my description: ${user!.description}");
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 10),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  _launchEmail() async {
    if (await canLaunch("mailto:info@jobreels.app")) {
      await launch("mailto:mailto:info@jobreels.app");
    } else {
  ...
  Widget drawerWidget() {
    return Container(
      ...
      child: Get.find<AuthController>().getLoginUserData()!.type.toString() ==
              "admin"
          ? Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
              ...
              children: [
                ...
                drawerButton(
                  ...
                  onPressed: () {
                    _launchEmail();
                    // Get.to(() => const WebViewScreen(
                    //       url: AppString.feedBackUrl,
                    //       title: AppString.feedBack,
                    //     ));
                  },
                ),
                if (Get.find<AuthController>().isLoginUserSubscribed)
                  user == null
                      ? Container(
                          height: 0,
                          width: 0,
                      ...
                      : user!.isFreelancer
                          ...
                          : drawerButton(
                              ...
                              title: AppString.cancelSubscription,
                              onPressed: () {
                                showPopUpAlert(
                                  popupObject: PopupObject(
                                    title: "Confirm Manage Subscription",
                                    body:
  ...
  Future<void> onEditProfilePressed() async {
    ...
    await Get.to(() => user!.isFreelancer
        ? const FreeLancerRegisterUpdateScreen()
        : const HirerRegisterUpdateScreen());
    user = Get.find<AuthController>().getLoginUserData()!;
    setState(() {});
  }
  ...
  Future getUserProfileData() async {
    ...
    Map<String, dynamic> response = await (widget.userId == null
        ? Get.find<AuthController>().getUserProfile()
        : Get.find<AuthController>().getOtherUserProfile(widget.userId!));
    if (response.containsKey(API_RESPONSE.SUCCESS)) {
      Map<String, dynamic> result = response[API_RESPONSE.SUCCESS];
      print("result['data'].........>${result['data']['freelancer']}");
      setState(() {
        user = User.fromJson(result['data']);
      });
