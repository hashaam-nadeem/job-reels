...
class API_STRUCTURE {
  ...
  Future<Map<String, dynamic>> requestAPI({bool isShowLoading = false,bool isCheckAuthorization=true}) async {
    ...
    try {
      api = AppConstant.BASE_URL + apiUrl;
  ...
  Future<Map<String, dynamic>> downloadApiRequest({bool isShowLoading = false,bool isCheckAuthorization=true}) async {
    var dir = await DownloadsPathProvider.downloadsDirectory;
    dynamic savePath;
    if(dir != null){
      String savename = Get.find<ShiftController>().fileName??"";
    ...
    try {
      ...
      /// Else for Delete Method
          :  apiRequestMethod == API_REQUEST_METHOD.DELETE
          ? await dio.delete(api, options: options)
          :apiRequestMethod == API_REQUEST_METHOD.PUT
          ?await dio.put(api, data: body, options: options)
          :await dio.download(
          api,
          savePath,
      ...
      debugPrint('$api response:[${response.statusCode}]---->data ${(response.data as ResponseBody).statusMessage}');
      if (isShowLoading){
        ApiLoader.hide();
      }
      if(response.statusCode==200){
